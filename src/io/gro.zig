...
test "write GRO round-trip" {
    ...
    for (0..result.topology.atoms.len) |i| {
        try std.testing.expectApproxEqAbs(result.frame.x[i], result2.frame.x[i], 0.05);
        try std.testing.expectApproxEqAbs(result.frame.y[i], result2.frame.y[i], 0.05);
        try std.testing.expectApproxEqAbs(result.frame.z[i], result2.frame.z[i], 0.05);
    }
}
