...
/// Write a Topology + Frame as GRO format.
/// Coordinates are converted from Angstroms to nm (÷10).
pub fn write(
    writer: anytype,
    ...
    // Line 1: Title
    if (frame.time != 0.0) {
        try writer.print("Generated by ztraj, t={d:10.5}\n", .{frame.time});
    } else {
        try writer.writeAll("Generated by ztraj\n");
    }
    ...
    for (topology.atoms, 0..) |atom, i| {
        ...
        const resid_raw: u32 = @intCast(@max(residue.resid, 0));
        const resid_wrapped: u32 = resid_raw % 100000;
        const serial: u32 = @intCast((i + 1) % 100000);

        const res_name = residue.name.slice();
        const atom_name = atom.name.slice();

        const x_nm = frame.x[i] / 10.0;
        const y_nm = frame.y[i] / 10.0;
        const z_nm = frame.z[i] / 10.0;

        // Cols 0-4: resid (%5d)
        // Cols 5-9: resname (%-5s, left-justified)
        // Cols 10-14: atom name (%5s, right-justified)
        // Cols 15-19: serial (%5d)
        // Cols 20-27: x (%8.3f), 28-35: y, 36-43: z
        try writer.print("{d:5}{s:<5}{s:>5}{d:5}{d:8.3}{d:8.3}{d:8.3}\n", .{
            resid_wrapped,
            res_name,
            atom_name,
            serial,
    ...
    if (frame.box_vectors) |box| {
        // Check if orthogonal (off-diagonals ~0)
        const eps: f32 = 1e-6;
        const is_orthogonal =
            @abs(box[0][1]) < eps and @abs(box[0][2]) < eps and
            @abs(box[1][0]) < eps and @abs(box[1][2]) < eps and
            @abs(box[2][0]) < eps and @abs(box[2][1]) < eps;

        if (is_orthogonal) {
            try writer.print("{d:10.5}{d:10.5}{d:10.5}\n", .{
                box[0][0] / 10.0,
                box[1][1] / 10.0,
                box[2][2] / 10.0,
            });
            ...
            // ztraj row-major: row0=(v1x,v1y,v1z), row1=(v2x,v2y,v2z), row2=(v3x,v3y,v3z)
            try writer.print(
                "{d:10.5}{d:10.5}{d:10.5}{d:10.5}{d:10.5}{d:10.5}{d:10.5}{d:10.5}{d:10.5}\n",
                .{
                    box[0][0] / 10.0, // v1x
                    box[1][1] / 10.0, // v2y
                    box[2][2] / 10.0, // v3z
        ...
        try writer.writeAll("   0.00000   0.00000   0.00000\n");
    }
}
...
test "write GRO round-trip" {
    ...
    for (0..result.topology.atoms.len) |i| {
        try std.testing.expectApproxEqAbs(result.frame.x[i], result2.frame.x[i], 0.02);
        try std.testing.expectApproxEqAbs(result.frame.y[i], result2.frame.y[i], 0.02);
        try std.testing.expectApproxEqAbs(result.frame.z[i], result2.frame.z[i], 0.02);
    ...
    // Box vectors should round-trip
    try std.testing.expect(result.frame.box_vectors != null);
    try std.testing.expect(result2.frame.box_vectors != null);
    const box1 = result.frame.box_vectors.?;
    const box2 = result2.frame.box_vectors.?;
    for (0..3) |r| {
        for (0..3) |c| {
            try std.testing.expectApproxEqAbs(box1[r][c], box2[r][c], 0.02);
        }
    }
}

test "parse alanine dipeptide GRO (22 atoms, 3 residues)" {
    const data =
        \\Generated by trjconv :  t=   0.00000
        \\   22
        \\    1ACE     H1    1   0.4290   1.3100   0.8590
        \\    1ACE    CH3    2   0.5200   1.3600   0.8800
        \\    1ACE     H2    3   0.4890   1.4300   0.9600
        \\    1ACE     H3    4   0.5600   1.4200   0.7890
        \\    1ACE      C    5   0.6100   1.2500   0.9400
        \\    1ACE      O    6   0.6400   1.2500   1.0600
        \\    2ALA      N    7   0.6590   1.1600   0.8490
        \\    2ALA      H    8   0.6500   1.1600   0.7500
        \\    2ALA     CA    9   0.7290   1.0400   0.9090
        \\    2ALA     HA   10   0.7890   1.0690   1.0000
        \\    2ALA     CB   11   0.6200   0.9500   0.9600
        \\    2ALA    HB1   12   0.5700   0.9090   0.8800
        \\    2ALA    HB2   13   0.6590   0.8700   1.0200
        \\    2ALA    HB3   14   0.5390   1.0000   1.0200
        \\    2ALA      C   15   0.8400   0.9790   0.8200
        \\    2ALA      O   16   0.8400   0.9890   0.7000
        \\    3NME      N   17   0.9300   0.9000   0.8800
        \\    3NME      H   18   0.9090   0.9000   0.9790
        \\    3NME      C   19   1.0500   0.8400   0.8300
        \\    3NME     H1   20   1.0690   0.7700   0.9090
        \\    3NME     H2   21   1.0400   0.7990   0.7290
        \\    3NME     H3   22   1.1300   0.9090   0.8300
        \\   1.00000   1.00000   1.00000
    ;
    const allocator = std.testing.allocator;
    var result = try parse(allocator, data);
    ...
    // 22 atoms, 3 residues (ACE, ALA, NME), 1 chain
    try std.testing.expectEqual(@as(usize, 22), result.topology.atoms.len);
    try std.testing.expectEqual(@as(usize, 3), result.topology.residues.len);
    try std.testing.expectEqual(@as(usize, 1), result.topology.chains.len);
    try std.testing.expectEqual(@as(usize, 22), result.frame.x.len);

    // Residue names
    try std.testing.expectEqualStrings("ACE", result.topology.residues[0].name.slice());
    try std.testing.expectEqualStrings("ALA", result.topology.residues[1].name.slice());
    try std.testing.expectEqualStrings("NME", result.topology.residues[2].name.slice());

    // Residue atom ranges
    try std.testing.expectEqual(@as(u32, 6), result.topology.residues[0].atom_range.len); // ACE: 6 atoms
    try std.testing.expectEqual(@as(u32, 10), result.topology.residues[1].atom_range.len); // ALA: 10 atoms
    try std.testing.expectEqual(@as(u32, 6), result.topology.residues[2].atom_range.len); // NME: 6 atoms

    // First atom: H1 at (0.4290, 1.3100, 0.8590) nm → (4.290, 13.100, 8.590) Å
    try std.testing.expectEqualStrings("H1", result.topology.atoms[0].name.slice());
    try std.testing.expectApproxEqAbs(@as(f32, 4.290), result.frame.x[0], 0.01);
    try std.testing.expectApproxEqAbs(@as(f32, 13.100), result.frame.y[0], 0.01);
    try std.testing.expectApproxEqAbs(@as(f32, 8.590), result.frame.z[0], 0.01);

    // Last atom: H3 at (1.1300, 0.9090, 0.8300) nm → (11.300, 9.090, 8.300) Å
    try std.testing.expectApproxEqAbs(@as(f32, 11.300), result.frame.x[21], 0.01);
    try std.testing.expectApproxEqAbs(@as(f32, 9.090), result.frame.y[21], 0.01);
    try std.testing.expectApproxEqAbs(@as(f32, 8.300), result.frame.z[21], 0.01);

    // Box vectors: 1.0 nm → 10.0 Å (orthogonal)
    try std.testing.expect(result.frame.box_vectors != null);
    const box = result.frame.box_vectors.?;
    try std.testing.expectApproxEqAbs(@as(f32, 10.0), box[0][0], 0.01);
    try std.testing.expectApproxEqAbs(@as(f32, 10.0), box[1][1], 0.01);
    try std.testing.expectApproxEqAbs(@as(f32, 10.0), box[2][2], 0.01);

    // Time from title: t= 0.00000
    try std.testing.expectApproxEqAbs(@as(f32, 0.0), result.frame.time, 0.01);
