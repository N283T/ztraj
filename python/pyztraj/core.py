...
def load_pdb(path: str | Path) -> Structure:
    ...
    _check(lib.ztraj_load_pdb(path_bytes, handle_ptr))
    handle = handle_ptr[0]
...
class XtcReader:
    ...
    def __enter__(self) -> XtcReader:
        ...
        _check(self._lib.ztraj_open_xtc(self._path, n_atoms_out, handle_ptr))
...
def compute_distances(
    ...
    _check(get_lib().ztraj_distances(
...
def compute_dihedrals(
    ...
    _check(get_lib().ztraj_dihedrals(
        _ptr_f32(x), _ptr_f32(y), _ptr_f32(z),
...
def compute_center_of_mass(
    ...
    cy = ffi.new("double*")
    cz = ffi.new("double*")
    _check(get_lib().ztraj_center_of_mass(
        _ptr_f32(x), _ptr_f32(y), _ptr_f32(z),
...
def compute_center_of_geometry(
    ...
    cy = ffi.new("double*")
    cz = ffi.new("double*")
    _check(get_lib().ztraj_center_of_geometry(
        _ptr_f32(x), _ptr_f32(y), _ptr_f32(z),
